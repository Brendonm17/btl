...
- **BTL-to-C transpiler** - compile BTL scripts to optimized C for native performance
...
BTL includes a transpiler that compiles BTL scripts into optimized C code. The generated C is then compiled with a standard C compiler (GCC/Clang) to produce a native binary, giving significant performance improvements over the bytecode interpreter.
...
```bash
make transpiler   # Build the transpiler tool
```
