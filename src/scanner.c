...
static TokenType identifierType(Scanner* scanner) {
    ...
    case 'd':
        if (scanner->current - scanner->start > 1) {
            ...
            case 'e': return checkKeyword(scanner, 2, 5, "fault", TOKEN_DEFAULT);
            }
        }
        break;
...
Token scanToken(Scanner* scanner) {
    ...
    case '+':
        return makeToken(scanner, match(scanner, '+') ? TOKEN_PLUS_PLUS : match(scanner, '=') ? TOKEN_PLUS_EQUAL : TOKEN_PLUS);
    case '-':
        return makeToken(scanner, match(scanner, '-') ? TOKEN_MINUS_MINUS : match(scanner, '=') ? TOKEN_MINUS_EQUAL : TOKEN_MINUS);
    case '*':
